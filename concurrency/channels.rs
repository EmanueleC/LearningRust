...
fn main() {
    let (on_sd, on_rcv) : (SyncSender<i32>, Receiver<i32>) = sync_channel(1);
    let (off_sd, off_rcv) : (SyncSender<i32>, Receiver<i32>) = sync_channel(1);
    ...
    thread::spawn(move || { counter_prisoner(10, on_sdx); }).join();
}
